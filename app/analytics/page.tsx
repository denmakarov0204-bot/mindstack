...
export default async function AnalyticsPage() {
  ...
  return (
    <div className="p-8 animate-fade-in max-w-[1100px]">
      <div className="mb-7">
        <h1 className="text-2xl font-extrabold tracking-tight">Аналитика</h1>
        <p className="text-[13px] text-muted mt-1">Статистика группы за последние 30 дней</p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label: 'Заполняемость', value: `${reportRate}%`, color: reportRate >= 80 ? 'text-c-green' : 'text-c-red', sub: `${submittedReports}/${totalReports} отчётов` },
          { label: 'Касса штрафов', value: `${totalBank.toLocaleString('ru')}₽`, color: 'text-c-orange', sub: `долг ${totalDebt.toLocaleString('ru')}₽` },
          { label: 'Встреч проведено', value: String(completedMeetings), color: 'text-c-blue', sub: `из ${meetings.length} всего` },
        ].map(kpi => (
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        {/* Штрафы по участникам */}
        <div className="bg-surface border border-border rounded-2xl p-5">
          <div className="text-[14px] font-bold mb-4">💸 Штрафы по участникам</div>
          {balances.map((m: any) => (
          ...
          <div className="grid grid-cols-3 gap-3">
            {top && (
              <div className="bg-surface2 rounded-xl p-3 border-l-2 border-c-green">
                <div className="text-[11px] text-c-green font-bold uppercase mb-1">Лидер группы</div>
                <div className="text-[13px]">{top.name} держит высший балл дисциплины — {top.score}.</div>
              </div>
            )}
            {bottom && bottom.score < 80 && (
              <div className="bg-surface2 rounded-xl p-3 border-l-2 border-c-red">
                ...
                <div className="text-[13px]">{bottom.name.split(' ')[0]} — балл {bottom.score}. Нужно подтянуть.</div>
              </div>
            )}
            <div className="bg-surface2 rounded-xl p-3 border-l-2 border-accent">
              <div className="text-[11px] text-accent2 font-bold uppercase mb-1">Заполняемость</div>
              <div className="text-[13px]">
                {reportRate >= 80 ? `Группа в хорошем ритме — ${reportRate}% отчётов сданы.` : `Заполняемость ${reportRate}% — ниже нормы 80%.`}
              </div>
            </div>
          </div>
        </div>
