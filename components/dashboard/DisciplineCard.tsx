...
export default function DisciplineCard({ scores }: { scores: DisciplineScore[] }) {
  return (
    <div className="bg-surface border border-border rounded-2xl p-5">
      ...
      {scores.map((s, i) => (
        <div key={s.member_id} className="flex items-center gap-2.5 py-2 border-b border-border last:border-0">
          ...
          <div className="flex-1 h-1.5 bg-surface2 rounded-full overflow-hidden mx-2">
            <div style={{
              width: `${s.score}%`,
              background: s.score >= 80
                ? 'linear-gradient(90deg,#7c6aff,#a78bfa)'
                : 'linear-gradient(90deg,#ff5566,#ff8866)'
            }} />
          </div>
          <div className={`text-[13px] font-bold font-mono min-w-[32px] text-right ${s.score >= 80 ? 'text-c-green' : 'text-c-red'}`}>
            {s.score}
          </div>
        </div>
      ))}
