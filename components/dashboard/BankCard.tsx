  ...
  return (
    <div className="bg-surface border border-border rounded-2xl p-5">
      ...
      <div className="text-center py-4 border-b border-border mb-3.5">
        <div className="text-[11px] text-muted font-mono uppercase">Общий банк</{totalBank.toLocaleString('ru')}₽</div>
        <div className="text-[12px] text-muted mt-1">Оплачено {totalPaid.toLocaleString('ru')}₽ · Долг {totalDebt.toLocaleString('ru')}₽</div>
      </div>
      {fineBalances.map(m => (
        <div key={m.id} className="flex items-center py-2 border-b border-border last:border-0">
          <div style={{ background: 'rgba(124,106,255,0.15)', color: '#a78bfa' }}>
            {m.name.charAt(0)}
          </div>
          ...
          <div className="flex-1 mx-3 h-1 bg-surface2 rounded-full overflow-hidden">
            <div style={{ width: `${(m.debt / maxDebt) * 100}%`, background: m.debt === 0 ? 'transparent' : m.debt > 500 ? '#ff5566' : '#f59e0b' }} />
          </div>
          <div className={`text-[12px] font-mono font-semibold min-w-[60px] text-right ${m.debt === 0 ? 'text-muted' : m.debt > 500 ? 'text-c-red' : 'text-c-orange'}`}>
